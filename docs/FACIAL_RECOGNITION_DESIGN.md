...
SquareGuardian จะมีระบบ Facial Recognition แบบ **Local-first** โดยมีแนวทางหลัก:

1. **SquareGuardian FR (หลัก)** — ระบบจดจำใบหน้าของเราเอง (InsightFace/ArcFace) รันบน server — ใช้ได้กับ **ทุกกล้อง RTSP**
...
> **หลักการสำคัญ:** SquareGuardian FR ทำได้เองทั้งหมด ไม่ต้องพึ่งกล้องยี่ห้อใดยี่ห้อหนึ่ง
...
## Part 1: SquareGuardian FR (ระบบของเราเอง)
...
> **หลักการ:** SquareGuardian FR ทำ face recognition ได้เองทั้งหมดผ่าน Frigate + RTSP
...
**ใช้ IsPeople เป็น trigger** → เมื่อกล้อง detect person → grab RTSP frame → ส่งให้ SquareGuardian FR:
...
onvif-trigger:
  ...
  environment:
    ...
    - SQUAREGUARDIAN_URL=http://squareguardian:8080
  depends_on:
    - face-service
  restart: unless-stopped
```
...
เมื่อพร้อม → สร้าง tapo-bridge service ที่:
  ...
  2. Sync ชื่อคนที่จำได้เข้า SquareGuardian gallery
  3. Cross-validate กับ SquareGuardian FR
```
...
หลักการ:
  → SquareGuardian FR ทำได้เองทั้งหมดอยู่แล้ว
...
squareguardian/
├── services/
│   ├── face/                          # SquareGuardian FR service
│   │   ├── Dockerfile
...
| ด้าน | SquareGuardian FR (ของเรา) | Tapo FR (ในตัวกล้อง) |
|------|--------------------------|---------------------|
...
- **SquareGuardian FR ทำได้เองทั้งหมด** → face recognition ครบบน server ของเรา ใช้กับทุกกล้อง RTSP
