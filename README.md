...
- รันด้วย `docker compose up -d` โดยไม่ต้อง build service เสริมด้าน identity ก่อน
- ใช้กล้อง RTSP 1 ตัว
...
สคริปต์นี้ตั้งใจเขียนให้ใช้ได้กับ Windows PowerShell 5.1 จาก `dev-up.cmd` โดยไม่พึ่งอักขระพิเศษในไฟล์ เพื่อเลี่ยงปัญหา parser error บนเครื่อง Windows บางชุด.
...
3. รัน `docker compose up -d --build` สำหรับ MVP detection stack
...
## Identity / face-service เป็น optional profile

เพื่อให้ local pilot ฝั่ง detection เริ่มได้เสถียรก่อน ตอนนี้ `dev-up` และ `docker compose up -d --build` จะ **ไม่ build `face-service` ตามค่าเริ่มต้น**. Service นี้เป็น optional profile สำหรับงาน `identity` ซึ่งยังอยู่นอก MVP รอบแรก.

ถ้าต้องการเปิด face-service เพิ่มเอง ให้รัน:

```bash
docker compose --profile identity up -d --build
```

และถ้าต้องการให้ `squareguardian` เรียกใช้งาน face-service ด้วย ให้กำหนดค่าใน `.env` เพิ่มอีกบรรทัด:

```env
FACE_SERVICE_URL=http://face-service:8082
```

ถ้าไม่ได้เปิด profile นี้ไว้ ระบบหลักจะยังรัน detection, zone และ event flow ได้ตามปกติ โดย identity endpoint จะตอบว่า face-service ยังไม่ถูกตั้งค่าแทน.
